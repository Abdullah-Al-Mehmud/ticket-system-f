...
const TicketTypeEdit = () => {
  ...
  const { data: ticketTypeData, isLoading: isFetching, isError: fetchError } = useGetTicketTypeByIdQuery(id);
  const [updateTicketType, { isLoading }] = useUpdateTicketTypeMutation();
  ...
  useEffect(() => {
    if (ticketTypeData?.data) {
      const ticketType = ticketTypeData.data;
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await updateTicketType({ id, ...formData }).unwrap();
      toast.success(res.message || "Ticket type updated successfully!");
      navigate("/admin/ticket-types-list");
    } catch (err) {
      toast.error(err?.data?.message || "Failed to update ticket type");
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white p-8 rounded-2xl shadow-sm border text-center max-w-md">
          ...
          <p className="text-gray-600">Unable to load ticket type. Please try again.</p>
          <button
      ...
      <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-sm border p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-4 pt-4">
            ...
            >
              {isLoading ? "Updating..." : "Update Ticket Type"}
            </button>
          </div>
        </form>
      </div>
